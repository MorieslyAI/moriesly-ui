...
interface GlycationScannerProps {
  ...
  onScan: (file: File | null, snapshot: FaceLandmarkSnapshot | null, base64?: string) => void;
  isLoading: boolean;
  onClose: () => void;
}
    ...
    <div className="absolute inset-0 z-40 bg-black/80 flex flex-col items-center justify-center p-6 backdrop-blur-sm">
      <div className="relative w-48 h-48 mb-8">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <svg className="w-16 h-16 text-teal-500 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
// ─── Solution card ────────────────────────────────────────────────────────────
const SolutionCard: React.FC<{ title: string; content: string; icon: string; color: string }> = ({
  ...
  <div className={`bg-white/5 border-l-2 ${color} p-4 rounded-r-xl mb-3 shadow-sm animate-in slide-in-from-right-4 duration-500`}>
    ...
    <p className="text-sm text-zinc-200 leading-relaxed font-medium">{content}</p>
  </div>
);
...
const FaceMap: React.FC<FaceMapProps> = ({ zones, onSelectZone, activeZoneArea, overlayStyle, isMirrored }) => {
  const getFallback = (area: string) => {
    ...
    if (a.includes('chin') || a.includes('jaw')) return { top: '77%', left: '50%' };
    ...
    <div className="absolute pointer-events-none" style={overlayStyle}>
      {zones.map((zone, i) => {
        const coordX = zone.coordinates
          ? (isMirrored ? 100 - zone.coordinates.x : zone.coordinates.x)
          : null;
        const coordY = zone.coordinates ? zone.coordinates.y : null;

        const top = coordY !== null ? `${coordY}%` : getFallback(zone.area).top;
        const left = coordX !== null ? `${coordX}%` : getFallback(zone.area).left;
        ...
        return (
          <button
            ...
            style={{ top, left }}
          >
            <div className={`absolute inset-0 rounded-full ${color} ${isActive ? 'animate-ping opacity-70' : 'opacity-0'} duration-1000`} />
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-center mb-1">
                ...
                <span className={`w-2 h-2 rounded-full flex-shrink-0 ${isHigh ? 'bg-rose-500' : zone.severity === 'Medium' ? 'bg-amber-500' : 'bg-teal-500'}`} />
              </div>
...
const GlycationScanner: React.FC<GlycationScannerProps> = ({
  ...
  const [imageBox, setImageBox] = useState<{ width: number; height: number; left: number; top: number } | null>(null);
  const [imageNatural, setImageNatural] = useState<{ width: number; height: number } | null>(null);
  const [lastSnapshot, setLastSnapshot] = useState<FaceLandmarkSnapshot | null>(null);
  ...
  const handleCapture = useCallback(() => {
    ...
    const video = document.querySelector<HTMLVideoElement>('video[playsinline]');
    if (!video) return;
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-zinc-950 flex flex-col items-center justify-center p-0 md:p-6 animate-in fade-in duration-300 overflow-hidden">
      ...
      <div className="w-full h-full flex flex-col relative">
        ...
        <div className={`relative w-full transition-all duration-500 ease-in-out overflow-hidden bg-black ${data ? 'h-[40%] md:h-[45%]' : 'h-full'}`}>
            ...
            <div ref={frameRef} className="relative w-full h-full">
              ...
              {data?.faceZones && (
                <FaceMap
                  ...
                  onSelectZone={zone => { setActiveZoneArea(zone.area); setActiveTab('scan'); }}
              ...
              {!isLoading && (
                <div className="absolute bottom-6 left-0 right-0 flex flex-col items-center gap-3 z-20">
                  ...
                  <button
                    ...
                    className={`relative w-20 h-20 rounded-full border-4 flex items-center justify-center transition-all active:scale-95 shadow-xl ${faceDetected
                      ? 'border-teal-400/80 shadow-teal-900/50 hover:scale-105'
                      : 'border-zinc-600/50 opacity-50 cursor-not-allowed'
                      }`}
                  >
                    <div className={`w-16 h-16 rounded-full flex items-center justify-center transition-colors ${faceDetected ? 'bg-teal-500 hover:bg-teal-400' : 'bg-zinc-700'
                      }`}>
                  ...
                  <p className="text-[10px] text-zinc-500 font-mono uppercase tracking-widest">
                    {faceDetected ? 'Tap to Scan' : 'Position your face in frame'}
                  </p>
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300 pb-20">
                  {data.recommendations.emergencyFix && (
                    <div className="bg-rose-600 p-4 rounded-xl shadow-lg shadow-rose-900/50 mb-4">
                      ...
                      <p className="text-sm font-bold text-white leading-tight">{data.recommendations.emergencyFix}</p>
                    </div>
                  )}

                  <SolutionCard title="Active Skincare Regime" content={data.recommendations.skincare} icon="🧪" color="border-purple-500" />
                  <SolutionCard title="Dietary Counter-Measure" content={data.recommendations.diet} icon="🥗" color="border-emerald-500" />
                  <SolutionCard title="Lifestyle Correction" content={data.recommendations.habit} icon="⚡" color="border-amber-500" />
                  ...
                  <div className="mt-6 pt-4 border-t border-zinc-800">
                    ...
                    <div className="flex justify-between items-center text-[9px] text-zinc-400">
                      {[['24h', 'Depuff'], ['7d', 'Clarity'], ['30d', 'Glow']].map(([label, desc], i, arr) => (
                        <React.Fragment key={label}>
                          <div className="flex flex-col items-center gap-1">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-white border ${i === arr.length - 1
                              ? 'bg-teal-900/50 border-teal-500/50 text-teal-400'
                              : 'bg-zinc-800 border-zinc-700'
                              }`}>{label}</div>
                            <span>{desc}</span>
                          </div>
                          {i < arr.length - 1 && <div className="h-0.5 flex-1 bg-zinc-800 mx-2" />}
                        </React.Fragment>
                      ))}
                    </div>
                  </div>
