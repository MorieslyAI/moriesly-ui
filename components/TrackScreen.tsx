...
const Skeleton: React.FC<{ className?: string }> = ({ className = "" }) => (
  <div
    className={`animate-pulse bg-zinc-200 dark:bg-zinc-700 rounded-lg ${className}`}
  />
);
...
const TrackScreen: React.FC<TrackScreenProps> = ({ onSetBackHandler }) => {
  ...
  const weightChartData = useMemo(() => {
    ...
    return trackData.weightHistory.map((entry) => ({
      date: new Date(entry.date + "T00:00:00").toLocaleDateString(undefined, {
        month: "short",
        day: "numeric",
      }),
      weight: entry.weight,
  ...
  const renderError = (msg: string, onRetry: () => void) => (
    <div className="flex flex-col items-center gap-4 py-16">
      ...
      <button
        onClick={onRetry}
        className="flex items-center gap-2 px-4 py-2 bg-brand-500 text-white rounded-full text-sm font-bold"
      >
  ...
  const macroData = trackData
    ? [
        {
          name: "Protein",
          value: trackData.todayMacros.protein,
          color: "#3b82f6",
        },
    ...
    <div className="pb-24 animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
      ...
      {activeTab === "body" &&
        (isLoadingData ? (
          ...
          renderError(dataError, fetchTrackData)
        ) : trackData ? (
          <div className="space-y-6">
            {/* Mission Card */}
            <div className="bg-brand-500 dark:bg-zinc-900 text-white rounded-3xl p-6 border border-brand-400 dark:border-zinc-800 shadow-xl relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="text-white/80 dark:text-brand-500 font-bold text-xs uppercase tracking-widest mb-1">
                  Active Mission
                </div>
                <h2 className="text-2xl font-black uppercase mb-4">
                  {trackData.goal.eventName}
                </h2>

                <div className="flex justify-between items-end mb-6">
                  <div>
                    <div className="text-4xl font-black">
                      {trackData.goal.currentWeight}
                      <span className="text-lg text-white/50 dark:text-zinc-500 dark:text-zinc-400">
                        kg
                      </span>
                    </div>
                    <div className="text-[10px] text-white/50 dark:text-zinc-500 dark:text-zinc-400 uppercase">
                      Current Weight
                    </div>
                  </div>
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-3xl p-6 border border-zinc-200 dark:border-zinc-800 shadow-sm h-64">
              ...
              {weightChartData.length > 1 ? (
                ...
                <div className="flex items-center justify-center h-full text-zinc-400 text-sm">
                  Log more weight to see the trend.
                </div>
              )}
            </div>
          </div>
                  ...
                  <Bar dataKey="sugar" radius={[4, 4, 0, 0]}>
                    {trackData.sugarLast7Days.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={
                          entry.sugar > trackData.sugarLimit
                            ? "#ef4444"
                            : "#33ADAE"
                        }
                      />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-white dark:bg-zinc-900 rounded-3xl p-6 border border-zinc-200 dark:border-zinc-800 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-sm font-bold uppercase text-zinc-500 dark:text-zinc-400">
                    Calories
                  </h3>
                ...
                <div className="flex items-end gap-2">
                  <span className="text-3xl font-black text-zinc-900 dark:text-white">
                    {trackData.todayMacros.calories}
                  </span>
                  <span className="text-xs text-zinc-400 mb-1">
                    / {trackData.calorieTarget} kcal
                  </span>
                </div>
                <div className="w-full bg-zinc-100 dark:bg-zinc-800 h-2 rounded-full mt-2 overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min((trackData.todayMacros.calories / trackData.calorieTarget) * 100, 100)}%`,
                    }}
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {trackData.todayMacros.vitamins.length > 0 ? (
                  trackData.todayMacros.vitamins.map((vit, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-100 dark:border-zinc-800"
                    >
                  ...
                  <div className="col-span-2 text-center text-xs text-zinc-400 py-4">
                    No vitamin data recorded today.
                  </div>
                )}
              </div>
            </div>
            ...
            {isLoadingNotes ? (
              ...
              <div className="space-y-3">
                {notes.map((note) => (
                  <div
                    key={note.id}
                    className="p-4 bg-zinc-50 dark:bg-zinc-800/30 rounded-2xl border border-zinc-100 dark:border-zinc-800 flex justify-between items-start group"
                  >
                    <div>
