...
const PsychoProfile: React.FC<PsychoProfileProps> = ({ history, limit }) => {
  const [profileMode, setProfileMode] = useState<"weakness" | "cycle">(
    "weakness",
  );
  ...
  const tasteStats = useMemo(() => {
    ...
    const days = Math.max(
      1,
      new Set(consumed.map((h) => new Date(h.timestamp).toDateString())).size,
    );
  ...
  const projection = useMemo(() => {
    ...
    const oldest = new Date(
      Math.min(...consumed.map((h) => new Date(h.timestamp).getTime())),
    );
    const now = new Date();
      ...
      <div className="relative w-full aspect-square max-w-[240px] mx-auto mt-4">
        <svg viewBox="0 0 160 160" className="w-full h-full overflow-visible">
          ...
          <polygon
            points={points}
            fill={
              isWeakness ? "rgba(20, 184, 166, 0.2)" : "rgba(244, 63, 94, 0.2)"
            }
          ...
          {labels.map((l, i) => (
            <g key={i}>
              <text
                ...
                className="text-[8px] fill-zinc-500 dark:fill-zinc-400 font-bold uppercase tracking-wider"
              >
                {l.label}
              </text>
              <text
                ...
                textAnchor="middle"
                dominantBaseline="middle"
                className={`text-[7px] font-bold ${isWeakness ? "fill-teal-600 dark:fill-teal-500" : "fill-rose-600 dark:fill-rose-500"}`}
              >
                {l.val}%
              </text>
            </g>
        ...
        >
          ...
          <div className="relative z-10">
            <div className="text-3xl font-black text-zinc-900 dark:text-zinc-100 tracking-tighter leading-none mb-1">
              +{projection.yearProjectionKg.toFixed(1)}
              <span className="text-sm text-zinc-400 font-bold ml-0.5">kg</span>
            </div>
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-zinc-900 rounded-[2rem] border border-zinc-200 dark:border-zinc-700 shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto relative animate-in zoom-in-95 slide-in-from-bottom-4 duration-300"
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-zinc-200 dark:divide-zinc-800">
          ...
          <div className="p-8 relative overflow-hidden flex flex-col justify-between min-h-[350px] bg-gradient-to-br from-zinc-50 to-white dark:from-zinc-900 dark:to-black/50">
            ...
            <div className="relative z-10">
              <div className="text-xs font-bold uppercase text-zinc-400 dark:text-zinc-500 dark:text-zinc-400 tracking-wider mb-2">
                Future Projection
              </div>
              <h3 className="text-2xl font-bold text-zinc-900 dark:text-zinc-100 dark:text-white mb-2">
                The 1-Year Accumulation
              </h3>
              <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-6">
                If current daily average ({Math.round(projection.dailyAvg)}g)
                continues...
              </p>
              ...
              <div className="flex flex-col items-center justify-center py-4">
                <div className="text-6xl font-black text-zinc-900 dark:text-zinc-100 dark:text-white tracking-tighter mb-3 drop-shadow-lg">
                  {projection.yearProjectionKg.toFixed(1)}{" "}
                  <span className="text-2xl text-zinc-400 dark:text-zinc-500 dark:text-zinc-400">
                    kg
                  </span>
                </div>
            ...
            <div className="relative z-10 mt-auto pt-6 border-t border-zinc-200 dark:border-zinc-800 text-center">
              <div className="text-[10px] text-zinc-500 dark:text-zinc-400 uppercase font-mono">
                System Recommendation: Reduce Daily Avg by{" "}
                {Math.round(projection.dailyAvg * 0.2)}g.
              </div>
            </div>
          </div>
          ...
          <div className="p-8 relative flex flex-col min-h-[350px] bg-zinc-50/50 dark:bg-zinc-900/50">
            ...
            <div className="relative z-10 flex flex-col h-full">
              <div className="flex items-center justify-between mb-6">
                <div className="text-xs font-bold uppercase text-zinc-400 dark:text-zinc-500 dark:text-zinc-400 tracking-wider">
                  Psych-Ops Profile
                </div>
