...
import {
  ...
  ConsumptionTrigger,
  ConsultationSession,
  ReceiptAnalysis as ReceiptData,
  VersusResult,
  SkinAnalysis,
  FaceZone,
  DietPlan,
...
function App() {
  ...
  useEffect(() => {
    ...
    CapacitorApp.addListener("backButton", handleHardwareBack).then(
      (listener) => {
    ...
    try {
      if (saved) {
        ...
        return {
          ...defaultLedger,
          ...parsed,
          macros: { ...defaultLedger.macros, ...(parsed.macros || {}) },
  ...
  const [pendingItem, setPendingItem] = useState<PendingScanResult | null>(
    null,
  );
  ...
  const [barcodeResult, setBarcodeResult] = useState<BarcodeScanResult | null>(
    null,
  ); // NEW
  ...
  const [videoRemainingSeconds, setVideoRemainingSeconds] = useState<
    number | null
  >(null);
  const [videoRemainingCallsToday, setVideoRemainingCallsToday] = useState<
    number | null
  >(null);
  ...
  const normalizeHistoryItemForDashboard = (
    item: HistoryItem,
  ): HistoryItem & {
    date: string;
    type: string;
    ...
    const date =
      item.timestamp instanceof Date
        ? getLocalDateString(item.timestamp)
        : getLocalDateString(new Date(item.timestamp));

    return {
      ...item,

      // penting: backend query dashboard/history pakai field date
      date,

      // penting: backend service menghitung food/drink dari field type
      type: item.itemType || "food",

      // penting: sebagian backend membaca sugar, sebagian UI membaca sugarg
      sugar: item.sugarg || 0,

      // penting: backend home metrics membaca macro top-level
  ...
  const addHistoryItem = async (item: HistoryItem) => {
    ...
    try {
      // 2. Simpan ke backend
      await saveDashboardHistoryItem(normalizedItem);
    } catch (e) {
      console.error("Failed to save history to backend:", e);
    } finally {
      // 3. Paksa DashboardScreen fetch ulang dari backend
  ...
  const handleUpdateHistoryItem = async (updatedItem: HistoryItem) => {
    ...
    setLedger((prev) => {
      const oldMacros = oldItem.macros || {
        protein: 0,
        carbs: 0,
        fat: 0,
        fiber: 0,
      };
      const newMacros = updatedItem.macros || {
        protein: 0,
        carbs: 0,
        fat: 0,
        fiber: 0,
      ...
      return {
        ...
        macros: {
          protein:
            Math.round(
              (prev.macros.protein - oldMacros.protein + newMacros.protein) *
                10,
            ) / 10,
          carbs:
            Math.round(
              (prev.macros.carbs - oldMacros.carbs + newMacros.carbs) * 10,
            ) / 10,
          fat:
            Math.round((prev.macros.fat - oldMacros.fat + newMacros.fat) * 10) /
            10,
          fiber:
            Math.round(
              ((prev.macros.fiber || 0) -
  ...
  const sanitizeNutritionalData = (data: any) => {
    ...
    const sugar = Math.min(
      data.sugar || data.sugar_grams || data.hidden_sugar_grams || 0,
      500,
    );
        ...
        if (item) {
          const updatedItem = {
            ...
            macros: {
              protein:
                Math.round(
                  ((item.macros?.protein || 0) + (data.macros?.protein || 0)) *
                    10,
                ) / 10,
              carbs:
                Math.round(
                  ((item.macros?.carbs || 0) + (data.macros?.carbs || 0)) * 10,
                ) / 10,
              fat:
                Math.round(
                  ((item.macros?.fat || 0) + (data.macros?.fat || 0)) * 10,
                ) / 10,
              fiber:
                Math.round(
                  ((item.macros?.fiber || 0) + (data.macros?.fiber || 0)) * 10,
                ) / 10,
            },
          };
  ...
  const handleManualScan = async () => {
    ...
    try {
      // --- VERSUS MODE LOGIC ---
      if (scanMode === "versus") {
        const apiRes = await postVersusScan(
          versusImgA || base64Image,
          base64Image,
        ); // Use API
        if (apiRes.success && apiRes.data) {
          try {
          ...
          try {
            ...
            if (addOnTargetId) {
              ...
              if (item) {
                ...
                const updatedItem = {
                  ...
                  sugarg:
                    Math.round(
                      ((item.sugarg || 0) + dataSanitized.sugar) * 10,
                    ) / 10,
                  calories:
                    Math.round(
                      ((item.calories || 0) + dataSanitized.calories) * 10,
                    ) / 10,
                  hasAddOn: true,
                  macros: {
                    protein:
                      Math.round(
                        ((item.macros?.protein || 0) +
                          dataSanitized.macros.protein) *
                          10,
                      ) / 10,
                    carbs:
                      Math.round(
                        ((item.macros?.carbs || 0) +
                          dataSanitized.macros.carbs) *
                          10,
                      ) / 10,
                    fat:
                      Math.round(
                        ((item.macros?.fat || 0) + dataSanitized.macros.fat) *
                          10,
                      ) / 10,
                    fiber:
                      Math.round(
                        ((item.macros?.fiber || 0) +
                          dataSanitized.macros.fiber) *
                          10,
                      ) / 10,
                  },
                };
            ...
            if (scanMode === "receipt") {
              ...
              const totalActualSugarGrams = data.items
                ? data.items.reduce(
                    (acc: number, item: any) => acc + (item.sugarGrams || 0),
                    0,
                  )
                : 0;
              ...
              addHistoryItem({
                id: uuidv4(),
                name: "Label Scan",
                sugarg: Math.round((data.hidden_sugar_grams || 0) * 10) / 10,
                action: "scanned",
                itemType: "label",
                timestamp: new Date(),
              ...
              addHistoryItem({
                id: uuidv4(),
                name: data.product_name || "Barcode Scan",
                sugarg: Math.round((data.sugar_grams || 0) * 10) / 10,
                action: "scanned",
                itemType: "qr",
                timestamp: new Date(),
                aiVerdict: `Risk: ${data.risk_level}`,
                imageBase64: base64Image,
                metadata: data,
              });
              goBack();
  ...
  const handleReanalyze = async (
    ...
    try {
      const base64Image =
        ...
        uploadedImage ||
        videoFeedRef.current?.getSnapshot() ||
        undefined;

      const response = await api.postReanalyzeScan(
        targetName,
        targetType || "food",
        base64Image,
      );
    ...
    if (winner === "Reject") {
      setLedger((prev) => ({
        ...
        saved:
          Math.round(
            (prev.saved +
              (versusResult.itemA.sugar + versusResult.itemB.sugar) / 2) *
              10,
          ) / 10,
      ...
      // User Consumed one
      const chosenItem =
        winner === "A" ? versusResult.itemA : versusResult.itemB;
  ...
  const initiateDecision = (action: "consumed" | "rejected") => {
    ...
    if (action === "consumed") {
      console.log(
        `[DEBUG] initiateDecision: sugar amount is ${pendingItem.sugar}`,
      );
  ...
  const finalizeDecision = (
    ...
    console.log(
      `[DEBUG] finalizeDecision called with action: ${action}, trigger: ${trigger}`,
    );
    if (!pendingItem) {
    ...
    let itemCalories =
      Math.round(Math.min((pendingItem.calories || 0) * ratio, 5000) * 10) / 10;
    let itemProtein =
      Math.round(
        Math.min((pendingItem.macros?.protein || 0) * ratio, 500) * 10,
      ) / 10;
    let itemCarbs =
      Math.round(
        Math.min((pendingItem.macros?.carbs || 0) * ratio, 1000) * 10,
      ) / 10;
    let itemFat =
      Math.round(Math.min((pendingItem.macros?.fat || 0) * ratio, 500) * 10) /
      10;
    let itemFiber =
      Math.round(Math.min((pendingItem.macros?.fiber || 0) * ratio, 200) * 10) /
      10;
    let itemSugar =
      Math.round(Math.min(pendingItem.sugar * ratio, 500) * 10) / 10;
    ...
    if (action === "consumed") {
      ...
      if (
        ...
        itemCalories =
          Math.round((itemProtein * 4 + itemCarbs * 4 + itemFat * 9) * 10) / 10;
      }
      ...
      setLedger((prev) => {
        ...
        newVitamins.forEach((newVit) => {
          const existingIndex = updatedVitamins.findIndex(
            (v) => v.name === newVit.name,
          );
          if (existingIndex >= 0) {
      ...
      if (itemSugar > 15 && (pendingItem.glycemicIndex || 50) > 50) {
        ...
        crashDate.setMinutes(crashDate.getMinutes() + minutesToCrash);
        antidote = itemSugar > 30 ? "Walk 15 mins" : "Drink Water";
        setActiveCrash({
          crashTime: crashDate,
          severity: itemSugar > 40 ? "critical" : "moderate",
          antidote: antidote,
          isMitigated: false,
        });
      }
    ...
    const gl =
      pendingItem.glycemicIndex !== undefined
        ? Math.round((pendingItem.glycemicIndex * pendingItem.sugar) / 100)
        : undefined;
    ...
    setShowDeathWaiver(false);
  };

  // --- VOICE MODE RE-INTEGRATION ---
  const finalizeVideoSession = async (
    reason: string,
  ...
  const disconnectVoiceLink = async (
      ...
      await serviceRef.current.connect(
        {
          ...
          onMessage: (text, isUser) => {
            if (text) {
              ...
              setMissionLogs((prev) => {
                ...
                if (
                  lastLog &&
                  lastLog.sender === speaker &&
                  activeLogId.current === lastLog.id
                ) {
                  return prev.map((log) =>
                    log.id === lastLog.id
          ...
          onPolicyUpdate: (policy) => {
            ...
            if (policy.shouldEnd || policy.status === "ended") {
              updateStreamingLog(
                "spy",
                "Video call ended by policy: " +
                  (policy.reason || "limit reached"),
              );
  ...
  const handleDisconnect = async (
    transcriptOrReason?: Array<{ role: string; text: string }> | string,
  ) => {
  ...
  const handleMitigateCrash = () => {
    if (!activeCrash) return;
    setActiveCrash((prev) => (prev ? { ...prev, isMitigated: true } : null));
    addXp(30);
    updateStreamingLog("spy", "Crash Protocol Mitigated. Good job.");
  };
  const handlePayDebt = (amount: number) => {
    setLedger((prev) => ({
      ...prev,
      sugarDebt: Math.round(Math.max(0, prev.sugarDebt - amount) * 10) / 10,
    }));
    addXp(100);
    updateStreamingLog("spy", "Debt Cleared.");
  };
  ...
  if (showLegal) {
    return <LegalReminder onAccept={() => setShowLegal(false)} />;
  }
  if (!isSetupComplete) {
    return (
      <SetupScreen onComplete={handleSetupComplete} initialStats={userStats} />
    );
  }
    ...
    <div className="min-h-screen bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 pb-10 transition-colors duration-500">
      ...
      {showDeathWaiver && pendingItem && (
        <DeathWaiver
          ...
          onCancel={() => setShowDeathWaiver(false)}
        />
      )}
      {showTriggerModal && !showDeathWaiver && (
        <TriggerModal
          onSelect={(trigger, ratio) =>
            finalizeDecision("consumed", trigger, ratio)
          }
      ...
      >
        {currentView === "status" ? (
          ...
          <TrackScreen onSetBackHandler={setBackHandler} />
        ) : currentView === "explore" ? (
          <ExploreScreen
            userStats={userStats}
            ledger={ledger}
          ...
          />
        ) : currentView === "consultant" || currentView === "chat" ? (
          <ConsultantScreen
            userProfile={userStats}
            connectionState={connectionState}
            onConnect={handleConnect}
            onDisconnect={handleDisconnect}
            videoFeedNode={sharedVideoFeed}
            agentVolume={agentVolume}
            consultationHistory={consultationHistory}
            onSaveSession={(s) =>
              setConsultationHistory((prev) => [s, ...prev])
            }
            missionLogs={missionLogs}
            onFlipCamera={() => videoFeedRef.current?.flipCamera()}
            onToggleFlash={() => videoFeedRef.current?.toggleFlash()}
            transcript={transcript}
            onAddXp={addXp}
            onToggleFullScreen={setIsFullScreenVideo}
          />
        ) : currentView === "tracker" ? (
          <WeightTrackerScreen
          ...
          <div className="fixed inset-0 z-100 bg-black">
            ...
            {connectionState === ConnectionState.CONNECTED && transcript && (
              <div className="absolute bottom-24 left-4 right-4 z-40 flex justify-center pointer-events-none">
                <div
                  className={`max-w-[90%] px-4 py-3 rounded-2xl backdrop-blur-md border border-white/10 text-xs font-bold shadow-xl animate-in fade-in slide-in-from-bottom-2 ${transcript.isUser ? "bg-black/70 text-white/90 text-right ml-auto rounded-br-none" : "bg-teal-900/80 text-teal-50 mr-auto rounded-bl-none"}`}
                >
                  {transcript.text}
                </div>
              </div>
          ...
          <>
            ...
            {pendingItem || labelResult || barcodeResult || identifiedItem ? (
              <div className="px-4 pt-4 pb-24 min-h-screen bg-zinc-50 dark:bg-zinc-950">
                ...
                >
                  {pendingItem && pendingItem.sugar > 25 && (
                    <div className="absolute inset-0 border-4 border-rose-500/30 rounded-3xl pointer-events-none animate-pulse"></div>
                  )}
                  {labelResult ? (
                    <DeceptionDetector
                      data={labelResult}
                    ...
                    isConfirmingScan ? (
                      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col gap-4 items-center text-center py-4 md:py-8">
                        ...
                        <p className="text-xs md:text-sm text-zinc-500 dark:text-zinc-400">
                          AI has identified this item as:
                        </p>
                        ...
                        <div className="w-full flex flex-col gap-2 md:gap-3 mt-2 md:mt-4">
                          ...
                          <div className="relative flex items-center justify-center py-1 md:py-2">
                            ...
                            <span className="absolute bg-white dark:bg-zinc-900 px-2 text-[10px] text-zinc-400 font-bold uppercase">
                              OR
                            </span>
                          </div>
                          ...
                          <button
                            onClick={() => {
                              setIsConfirmingScan(false);
                              setManualName(
                                identifiedItem.name === "Unknown Item"
                                  ? ""
                                  : identifiedItem.name,
                              );
                              setManualType(identifiedItem.type);
                      ...
                      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col gap-6">
                        {identifiedItem.imageBase64 && (
                          <div className="w-full h-48 rounded-2xl overflow-hidden mb-2 shadow-sm border border-zinc-100 dark:border-zinc-800">
                            <img
                              src={`data:image/jpeg;base64,${identifiedItem.imageBase64}`}
                              alt="Scanned Item"
                        ...
                        {isCorrecting && (
                          <div className="bg-zinc-100 dark:bg-zinc-800 p-4 rounded-2xl animate-in fade-in slide-in-from-top-2">
                            <h3 className="text-sm font-bold uppercase text-zinc-500 mb-3">
                              Manual Correction
                            </h3>
                            <div className="flex flex-col gap-3">
                              <input
                                ...
                                className="w-full bg-white dark:bg-zinc-900 p-3 rounded-xl border border-zinc-200 dark:border-zinc-700 outline-none focus:border-teal-500 transition-colors"
                              />
                              <div className="flex gap-2">
                                <button
                                  onClick={() => setManualType("food")}
                                  className={`flex-1 py-2 rounded-xl text-xs font-bold uppercase ${manualType === "food" ? "bg-orange-100 text-orange-600 dark:bg-orange-500/20 dark:text-orange-400 ring-1 ring-orange-500" : "bg-zinc-200 dark:bg-zinc-700 text-zinc-500"}`}
                                >
                                  Food
                                </button>
                                <button
                                  onClick={() => setManualType("drink")}
                                  className={`flex-1 py-2 rounded-xl text-xs font-bold uppercase ${manualType === "drink" ? "bg-blue-100 text-blue-600 dark:bg-blue-500/20 dark:text-blue-400 ring-1 ring-blue-500" : "bg-zinc-200 dark:bg-zinc-700 text-zinc-500"}`}
                                >
                                  Drink
                                </button>
                              </div>
                              <div className="flex gap-2 mt-1">
                                <button
                                  ...
                                  className="flex-1 py-2.5 rounded-xl text-xs font-bold text-zinc-500 bg-transparent hover:bg-zinc-200 dark:hover:bg-zinc-700"
                                >
                                  Cancel
                                </button>
                                <button
                                  onClick={() => handleReanalyze()}
                                  disabled={!manualName || isScanning}
                                  className={`flex-1 py-2.5 rounded-xl text-xs font-bold text-white bg-teal-500 shadow-lg shadow-teal-500/20 ${!manualName || isScanning ? "opacity-50" : "hover:bg-teal-400"}`}
                                >
                                  {isScanning
                                    ? "Analyzing..."
                                    : "Analyze & Update"}
                        ...
                        {!isCorrecting && isScanning && (
                          <div className="flex flex-col items-center justify-center py-8 gap-4 animate-in fade-in duration-300">
                            ...
                            <div className="flex flex-col items-center gap-1">
                              <p className="text-sm font-black text-teal-500 uppercase tracking-widest animate-pulse">
                                Deep Analysis
                              </p>
                    ...
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col gap-6">
                      {pendingItem.imageBase64 && (
                        <div className="w-full h-32 md:h-48 rounded-2xl overflow-hidden mb-2 shadow-sm border border-zinc-100 dark:border-zinc-800">
                          <img
                            src={`data:image/jpeg;base64,${pendingItem.imageBase64}`}
                            alt="Scanned Item"
                            className="w-full h-full object-cover"
                          />
                        </div>
                      ...
                      <div className="grid grid-cols-3 gap-2 md:gap-3">
                        <div className="bg-linear-to-br from-zinc-50 to-zinc-100 dark:from-zinc-900 dark:to-zinc-950 p-2 md:p-3 rounded-2xl flex flex-col items-center justify-center border border-zinc-200/50 dark:border-zinc-800/50 shadow-sm relative overflow-hidden group">
                          <div className="absolute top-0 right-0 p-1 opacity-10 group-hover:opacity-20 transition-opacity">
                            <svg
                              className="w-6 h-6 md:w-8 md:h-8 text-zinc-500"
                              fill="currentColor"
                              viewBox="0 0 24 24"
                            >
                        ...
                        <div className="bg-linear-to-br from-zinc-50 to-zinc-100 dark:from-zinc-900 dark:to-zinc-950 p-2 md:p-3 rounded-2xl flex flex-col items-center justify-center border border-zinc-200/50 dark:border-zinc-800/50 shadow-sm relative overflow-hidden group">
                          ...
                          <div
                            className={`text-lg md:text-xl font-black z-10 ${
                              (pendingItem.glycemicIndex || 0) > 70
                                ? "text-rose-500"
                                : (pendingItem.glycemicIndex || 0) > 55
                                  ? "text-amber-500"
                                  : "text-emerald-500"
                            }`}
                          >
                      ...
                      {(pendingItem.macros ||
                        (pendingItem.vitamins &&
                          pendingItem.vitamins.length > 0)) && (
                          ...
                          {pendingItem.macros && (
                            <div className="mb-5">
                              ...
                              <div className="h-3 w-full bg-zinc-200 dark:bg-zinc-800 rounded-full overflow-hidden flex shadow-inner mb-4">
                                <div
                                  style={{
                                    width: `${(pendingItem.macros.protein / (pendingItem.macros.protein + pendingItem.macros.carbs + pendingItem.macros.fat + (pendingItem.macros.fiber || 0) || 1)) * 100}%`,
                                  }}
                                  className="h-full bg-blue-500"
                                ></div>
                                <div
                                  style={{
                                    width: `${(pendingItem.macros.carbs / (pendingItem.macros.protein + pendingItem.macros.carbs + pendingItem.macros.fat + (pendingItem.macros.fiber || 0) || 1)) * 100}%`,
                                  }}
                                  className="h-full bg-orange-500"
                                ></div>
                                <div
                                  style={{
                                    width: `${(pendingItem.macros.fat / (pendingItem.macros.protein + pendingItem.macros.carbs + pendingItem.macros.fat + (pendingItem.macros.fiber || 0) || 1)) * 100}%`,
                                  }}
                                  className="h-full bg-rose-500"
                                ></div>
                                {pendingItem.macros.fiber !== undefined &&
                                  pendingItem.macros.fiber > 0 && (
                                    <div
                                      style={{
                                        width: `${(pendingItem.macros.fiber / (pendingItem.macros.protein + pendingItem.macros.carbs + pendingItem.macros.fat + pendingItem.macros.fiber || 1)) * 100}%`,
                                      }}
                                  ...
                                  pendingItem.macros.fiber > 0 && (
                                    <div className="flex items-center gap-2.5 bg-white dark:bg-zinc-800/50 p-2.5 rounded-2xl border border-zinc-100 dark:border-zinc-700/50 shadow-sm">
                                      ...
                                      <div className="flex flex-col">
                                        ...
                                        <span className="text-sm font-black text-zinc-700 dark:text-zinc-200 leading-none">
                                          {pendingItem.macros.fiber.toFixed(1)}g
                                        </span>
                                      </div>
                                    </div>
                                  )}
                              ...
                              <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                  {pendingItem.vitamins.map((v, i) => (
                                    <div
                                      key={i}
                                      className="px-2 py-1.5 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg shadow-sm flex items-center gap-1.5"
                                    >
                                      <span className="text-[9px] font-bold text-zinc-500 dark:text-zinc-400 uppercase">
                                        {v.name}
                                      </span>
                                      <span className="text-[9px] font-black text-zinc-700 dark:text-zinc-300 bg-zinc-100 dark:bg-zinc-900 px-1 rounded">
                                        {v.percent}%
                                      </span>
                                    </div>
                                  ))}
                      ...
                      <div className="bg-zinc-50 dark:bg-zinc-800/50 p-4 rounded-2xl border border-zinc-100 dark:border-zinc-700/50 mb-6">
                        ...
                        {pendingItem.ingredients &&
                          pendingItem.ingredients.length > 0 && (
                            <div className="flex flex-wrap gap-2 mb-4">
                              {" "}
                              {pendingItem.ingredients.map((ing, i) => (
                                <span
                                  key={i}
                                  className="px-2 py-1 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-md text-[10px] font-bold text-zinc-600 dark:text-zinc-300 uppercase shadow-sm"
                                >
                                  {" "}
                                  {ing}{" "}
                                </span>
                        ...
                        <p className="text-xs md:text-sm text-zinc-700 dark:text-zinc-300 leading-relaxed italic bg-white/50 dark:bg-black/20 p-3 rounded-xl border border-zinc-200/50 dark:border-zinc-700/30">
                          {" "}
                          "{pendingItem.explanation}"{" "}
                        </p>
                      </div>

                      <EvidencePanel
                        confidence={pendingItem.confidence_score || 85}
                        sugarSources={pendingItem.sugar_sources || []}
                        visualCues={pendingItem.visual_cues || []}
                        dataRef={pendingItem.data_ref || "AI Pattern Matching"}
                        itemName={pendingItem.name}
                        sugarAmount={pendingItem.sugar}
                      />
                      <MetabolicInvoice
                        focusTax={pendingItem.focus_tax}
                        agingGrade={pendingItem.aging_grade as any}
                        sleepPenalty={pendingItem.sleep_penalty as any}
                      />
                      ...
                      <div className="space-y-4 mb-6">
                        <GlucosePredictor
                          ...
                          gi={pendingItem.glycemicIndex || 50}
                        />
                        <OrganMap
                          sugar={pendingItem.sugar}
                    ...
                    <div className="text-center py-4">
                      <div className="text-zinc-400 text-sm animate-pulse">
                        {isScanning
                          ...
                          : uploadedImage
                            ? "Image Loaded. Press Shutter to Analyze."
                            : "Align Target in Viewfinder"}
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div className="flex flex-col items-center gap-4 text-center">
              ...
              <div>
                ...
                <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
                  Are you sure you want to exit Moriesly AI?
                </p>
              </div>
